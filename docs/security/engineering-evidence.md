...
| Untrusted transport header input is fuzzed across the name, value, duplicate, and ordering decisions the header types actually make | `fuzz/corpus/transport_parts`; `fuzz/corpus/typed_headers`; `fuzz/tests/scenarios.rs::curated_generic_transport_seeds_reach_named_adapter_outcomes`; `fuzz/tests/scenarios.rs::curated_generic_typed_seeds_reach_named_construction_outcomes` | Required fuzz corpus contract |
