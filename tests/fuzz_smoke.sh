...
for target in transport_parts encoded_envelope typed_headers aead_envelope context_envelope; do
...
test "$(cat "$fixture/pristine_stable.events")" = "$(printf '%s\n' scenario fuzz:transport_parts fuzz:encoded_envelope fuzz:typed_headers fuzz:aead_envelope fuzz:context_envelope)" || \
...
test "$(wc -l <"$fixture/pristine_stable.log")" -eq 5 || fail "expected five smoke targets"
test "$(awk '{print $1}' "$fixture/pristine_stable.log")" = "$(printf '%s\n' transport_parts encoded_envelope typed_headers aead_envelope context_envelope)" || \
...
test "$(wc -l <"$fixture/extended.log")" -eq 5 || fail "expected five extended targets"
...
test "$(grep -Fc -- "-seed=424243" "$fixture/extended_seeded.log")" -eq 5 || \
