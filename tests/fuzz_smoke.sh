...
for target in transport_parts encoded_envelope typed_headers aead_envelope aead_ccm_envelope context_envelope; do
...
test "$(cat "$fixture/pristine_stable.events")" = "$(printf '%s\n' scenario fuzz:transport_parts fuzz:encoded_envelope fuzz:typed_headers fuzz:aead_envelope fuzz:aead_ccm_envelope fuzz:context_envelope)" || \
...
test "$(wc -l <"$fixture/pristine_stable.log")" -eq 6 || fail "expected six smoke targets"
test "$(awk '{print $1}' "$fixture/pristine_stable.log")" = "$(printf '%s\n' transport_parts encoded_envelope typed_headers aead_envelope aead_ccm_envelope context_envelope)" || \
...
test "$(wc -l <"$fixture/extended.log")" -eq 6 || fail "expected six extended targets"
...
test "$(grep -Fc -- "-seed=424243" "$fixture/extended_seeded.log")" -eq 6 || \
...
contains "$repo_root/fuzz/corpus/aead_ccm_envelope/full_valid_open" "vvv000|0:|0:|0:"
